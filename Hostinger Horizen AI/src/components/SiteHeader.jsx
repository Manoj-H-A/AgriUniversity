...
import { BookHeart, Menu, X, LogOut } from 'lucide-react';
...
const NavLink = ({ to, children, onClick, isHashLink = false }) => {
  ...
  return (
    <Link
      to={isHashLink ? (location.pathname === '/' ? `#${to.split('#')[1]}`: `/${to}`) : to}
      onClick={handleClick}
...
const SiteHeader = ({ isAuthenticated, onLogout, currentUser }) => {
  ...
  const headerNavLinks = isAuthenticated ? [
    { to: "/dashboard", label: "Dashboard", isHashLink: false }
  ] : [
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden md:flex space-x-2 items-center">
            {headerNavLinks.map(link => (
              <NavLink key={link.to} to={link.to} onClick={handleNavLinkClick} isHashLink={link.isHashLink}>{link.label}</NavLink>
            ))}
            {isAuthenticated && (
              <>
                <span className="text-slate-300 text-sm px-3 py-2">Welcome, {currentUser?.username}!</span>
      ...
      {isMobileMenuOpen && (
        ...
        >
          <nav className="flex flex-col space-y-2 px-4 pt-2">
            ...
            {isAuthenticated && (
               <>
                <span className="text-slate-300 text-sm px-3 py-2 text-center">Welcome, {currentUser?.username}!</span>
